...
import { getFineRecordsByCardID, getFineRecordsByID, payFine } from '@/apis/fines'
import { useMessage } from '@/utils'
import { FineRecord } from '@/utils/types'
...
const currentRecord: Partial<FineRecord> = reactive({})
const fineRecords: Partial<FineRecord>[] = reactive([])
...
function handleModalOK(done: (closed: boolean) => void) {
  ...
  getFineRecordsByID(form.fineID)
    .then(({data: res}) => {
...
<template>
  ...
  >
    ...
    >
      <a-option
        v-for="{id, record} in fineRecords"
        :key="id"
        :value="id"
      >
        {{ record }}
